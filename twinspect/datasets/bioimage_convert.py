...
Benchmark Collection archives. Each selected source bioimage becomes one cluster:
    ...
    0000000/4variant_jp2.jp2
    0000000/5variant_dicom.dcm
    0000000/6variant_ics.ics

The converted cluster members are intended for benchmarking IMAGEWALK-based
bioimage Data-Code matching across default storage-format conversions.
...
# Default-only single-file output formats. Directory formats such as OME-NGFF/Zarr
# are intentionally excluded because TwinSpect currently treats benchmark assets as
# files. The target set includes ordinary web/file formats plus more specialized
# Bio-Formats outputs (DICOM, ICS/IDS) to test storage-format conversion effects
# without synthetic pixel edits or custom codec/compression settings.
CONVERSIONS = (
